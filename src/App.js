...
class App extends Component {
  ...
  updateVisitedCountries = id => {
    this.setState(prevState => ({
      countriesList: prevState.countriesList.map(eachData => {
        if (id === eachData.id) {
          return {...eachData, isVisited: !eachData.isVisited}
        }
        return eachData
      }),
    }))
  ...
  render() {
    ...
    const filterCountries = countriesList.filter(
      eachData => eachData.isVisited === true,
    )
    ...
    return (
      <div className="main-bg-container">
        ...
        <ul className="ul-container">
          {countriesList.map(eachData => (
            <CountriesList
              countriesData={eachData}
